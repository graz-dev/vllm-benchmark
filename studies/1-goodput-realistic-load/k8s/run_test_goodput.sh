...
#
# --timeout=6000s (100m) kept from the inference-perf era even though AIPerf's
# closed-loop --concurrency model shouldn't reproduce the unbounded-duration failure
# that motivated it (see README) — cheap insurance, and still safely under the
# workflow's own RunTest task timeout (120m = 7200s, see
# akamas/1-Goodput-Realistic-Load-Workflow.yaml).
set +e
kubectl wait --for=condition=complete job/aiperf-benchmark -n llm-benchmark --timeout=6000s
WAIT_EXIT=$?
set -e
