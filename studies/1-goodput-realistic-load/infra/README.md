...
**This study deliberately targets the same cluster as `studies/0-explorative`**
(same name `vllm-bench`, same region, same A10G node group) — a decision, not an
oversight: this study validates a new load-testing tool
(`kubernetes-sigs/inference-perf`) and the vLLM pack's new parameters
(context-parallelism, speculative decoding) on already-proven hardware, before also
introducing new hardware (H100 + DRA, see `ROADMAP.md` Section D). `provision.sh`
detects the cluster already exists and skips creation if so — this folder is kept as
a full, independent copy anyway, per the atomic-per-study convention, in case this
study is ever run standalone on a fresh account.
...
```bash
cd studies/1-goodput-realistic-load/infra/eks
