...
# Provision the vllm-bench EKS cluster for this study (0-explorative).
#
# Node layout (all defined in cluster.yaml, created in one shot):
#   system      (m6i.xlarge,  4 vCPU / 16 GB)               — Prometheus, Grafana, GuideLLM
...
#
# This script is this study's own copy of the infra layer — deliberately not shared
# with other studies, so this study stays reproducible on its own even if a future
# study's cluster.yaml (e.g. a different instance type or node group layout) diverges.
#
...
echo ""
echo "=== vllm-bench EKS Cluster (studies/0-explorative) ==="
