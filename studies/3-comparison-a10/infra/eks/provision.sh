...
# Provision the vllm-bench EKS cluster for this study (3-comparison-a10).
#
...
# convention), but it deliberately targets the SAME cluster/node group as
# 0-explorative/1-goodput-realistic-load — this study exists specifically to re-run
# 2-larger-model-g7e's exact config back on this A10G hardware for a direct comparison,
# so reusing the identical node group is the point (unlike 2-larger-model-g7e, which
# added its own separate llm-serving-g7e group). If the cluster/node group are already
# up, this script detects it and skips creation.
#
...
echo ""
echo "=== vllm-bench EKS Cluster (studies/3-comparison-a10) ==="
