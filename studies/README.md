...
| [1-goodput-realistic-load](1-goodput-realistic-load/README.md) | DONE | vLLM (14/30 params, `spec_method`/`spec_tokens` dropped after 5 crashes), same Qwen2.5-7B-Instruct/1x A10G as `0-explorative`; goal = goodput (throughput + P95 TTFT/ITL SLA); AIPerf + real ShareGPT + 7-level concurrency sweep | Stopped at 32 experiments (user); **no optimize-step trial met the TTFT/ITL SLA** — baseline (2152 tokens/s) is the only SLA-compliant result. Best raw throughput 3131 tokens/s (+45.5%, `FLASHINFER`+`fp8`+high `max_num_seqs`) but misses TTFT by 500-900ms — SLA pair too strict for this hardware above baseline |
