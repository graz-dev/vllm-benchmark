DEPLOY_FILE=/work/vllm-benchmark/studies/7-tensor-parallelism/k8s/01-deployment.yaml
...
kubectl logs deployment/vllm -n llm-serving --all-containers --tail=200 || true
echo "--- vLLM container logs (previous pod, if it crashed and restarted) ---"
kubectl logs deployment/vllm -n llm-serving --all-containers --tail=200 --previous 2>/dev/null || true
